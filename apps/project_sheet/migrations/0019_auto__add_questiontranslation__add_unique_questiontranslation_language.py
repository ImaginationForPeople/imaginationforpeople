...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('project_sheet_questiontranslation', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('content', self.gf('django.db.models.fields.CharField')(max_length=512)),
            ('language_code', self.gf('django.db.models.fields.CharField')(max_length=15, db_index=True)),
            ('master', self.gf('django.db.models.fields.related.ForeignKey')(related_name='translations', null=True, to=orm['project_sheet.Question'])),
        ))
        db.send_create_signal('project_sheet', ['QuestionTranslation'])

        # Adding unique constraint on 'QuestionTranslation', fields ['language_code', 'master']
        db.create_unique('project_sheet_questiontranslation', ['language_code', 'master_id'])

        # Adding model 'TopicTranslation'
        db.create_table('project_sheet_topictranslation', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('label', self.gf('django.db.models.fields.CharField')(max_length=512)),
            ('language_code', self.gf('django.db.models.fields.CharField')(max_length=15, db_index=True)),
            ('master', self.gf('django.db.models.fields.related.ForeignKey')(related_name='translations', null=True, to=orm['project_sheet.Topic'])),
        ))
        db.send_create_signal('project_sheet', ['TopicTranslation'])

        # Adding unique constraint on 'TopicTranslation', fields ['language_code', 'master']
        db.create_unique('project_sheet_topictranslation', ['language_code', 'master_id'])

        # Adding model 'AnswerTranslation'
        db.create_table('project_sheet_answertranslation', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('content', self.gf('django.db.models.fields.TextField')()),
            ('language_code', self.gf('django.db.models.fields.CharField')(max_length=15, db_index=True)),
            ('master', self.gf('django.db.models.fields.related.ForeignKey')(related_name='translations', null=True, to=orm['project_sheet.Answer'])),
        ))
        db.send_create_signal('project_sheet', ['AnswerTranslation'])

        # Adding unique constraint on 'AnswerTranslation', fields ['language_code', 'master']
        db.create_unique('project_sheet_answertranslation', ['language_code', 'master_id'])

        # Deleting field 'Answer.content'
        db.delete_column('project_sheet_answer', 'content')

        # Deleting field 'Question.content'
        db.delete_column('project_sheet_question', 'content')

        # Deleting field 'Topic.language_code'
        db.delete_column('project_sheet_topic', 'language_code')

        # Deleting field 'Topic.label'
        db.delete_column('project_sheet_topic', 'label')


    def backwards(self, orm):
        
        # Removing unique constraint on 'AnswerTranslation', fields ['language_code', 'master']
        db.delete_unique('project_sheet_answertranslation', ['language_code', 'master_id'])

        # Removing unique constraint on 'TopicTranslation', fields ['language_code', 'master']
        db.delete_unique('project_sheet_topictranslation', ['language_code', 'master_id'])

        # Removing unique constraint on 'QuestionTranslation', fields ['language_code', 'master']
        db.delete_unique('project_sheet_questiontranslation', ['language_code', 'master_id'])

        # Deleting model 'QuestionTranslation'
        db.delete_table('project_sheet_questiontranslation')

        # Deleting model 'TopicTranslation'
        db.delete_table('project_sheet_topictranslation')

        # Deleting model 'AnswerTranslation'
        db.delete_table('project_sheet_answertranslation')

        # User chose to not deal with backwards NULL issues for 'Answer.content'
        raise RuntimeError("Cannot reverse this migration. 'Answer.content' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'Question.content'
        raise RuntimeError("Cannot reverse this migration. 'Question.content' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'Topic.language_code'
        raise RuntimeError("Cannot reverse this migration. 'Topic.language_code' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'Topic.label'
        raise RuntimeError("Cannot reverse this migration. 'Topic.label' and its values cannot be restored.")
